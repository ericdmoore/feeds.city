...
return (
 <nav class="border-b border-gray-200 bg-white">  {/* MenuBar */}
      ...
      <div class="flex h-16 justify-between">
        <div class="flex">          
          ...
          <div id='amIDesktopMenu' class="hidden sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
            ...
            <a href="#/projects"  class={`${css.nonMobile.common} ${css.nonMobile.inactive}`}>Projects</a>
            <a href="#/calender"  class={`${css.nonMobile.common} ${css.nonMobile.inactive}`}>Calendar</a>
          </div>
        </div>
        <div class="hidden sm:ml-6 sm:flex sm:items-center">
          ...
          <div class="relative ml-3">
            ...
            --> */}
            <div class={`${isOpen ? "" : "hidden"} absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none`} role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabIndex={-1}>
        ...
        <div class="-mr-2 flex items-center sm:hidden">
          ...
          <button 
            type="button" 
            onClick={()=>{setIsOpen(!isOpen)}}
            class="inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" aria-controls="mobile-menu" aria-expanded="false">
            <span class="sr-only">Open main menu</span>
            {/* <!-- Heroicon name: outline/bars-3   Menu open: "hidden", Menu closed: "block" --> */}
            <svg class={`${isOpen ? 'hidden' : 'block'} h-6 w-6`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
              <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
            </svg>
            {/* <!-- Heroicon name: outline/x-mark Menu open: "block", Menu closed: "hidden" --> */}
            <svg class={`${isOpen ? 'block' : 'hidden'} h-6 w-6`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
    ...
    <div class={`${isOpen ? "" : "hidden" } sm:hidden`} id="mobile-menu">
      <div class="space-y-1 pt-2 pb-3">
        ...
        <a href="#/home"      class={`${css.mobile.common} ${css.mobile.active}`} aria-current="page">Dashboard</a>
        <a href="#/team"      class={`${css.mobile.common} ${css.mobile.inactive}`}>Team</a>
        <a href="#/projects"  class={`${css.mobile.common} ${css.mobile.inactive}`}>Projects</a>
        <a href="#/calendar"   class={`${css.mobile.common} ${css.mobile.inactive}`}>Calendar</a>
      </div>
      {/* Mobile Profile */}
