...
export const s3stringer = (_config?: unknown )=>{
    ...
    const httpString = (input: S3UriParts, other: {region:string, webhosting: boolean} )=>{
        const usingCreds = input.creds?.key && input.creds?.secret 
        ...
        const usingQueryString = queryString.length > 0 ? 
            `?${queryString}` 
            : ""

        const usingWebhosting = other.webhosting 
            ? `s3-website`
            : 's3'
