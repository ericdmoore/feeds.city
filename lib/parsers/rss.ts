    ...
    fromAST: async (_ast: ASTcomputable | ASTjson): Promise<RespStruct> => {
      ...
      return {
        _declaration: {
          ...
          ...(ast._rss?._declaration
            ? ast._rss?._declaration as Record<string, unknown>
            : {}),
        },
        ...(ast._rss?._instruction
          ? ast._rss?._instruction as Record<string, unknown>
          : {}),
        rss: {
      ...
      return {
        ...
        links: async () => {
          return {
            ...
            list: [
              ...(
                c.rss.channel["atom:link"]
                  ? [c.rss.channel["atom:link"]._attributes]
                  : []
              ),
            ],
          };
            ...
            return {
              ...
              links: {
                ...
                tags: Array.isArray(i.category)
                  ? i.category.map((c) =>
                    txtorCData(">> couldNotFindTagInfo", c)
                  )
                  : i.category
                  ? [txtorCData("", i.category)]
                  : [],
              ...
              attachments: Array.isArray(i.enclosure)
                ? i.enclosure.filter((e) =>
                  e?._attributes.type && e?._attributes.url
                ).map((e) => {
                  return {
                    durationInSeconds: 0,
