		...
		toAST: async (): Promise<ASTcomputable> => {
			...
			return {
				...
				links: () => {
					...
					const homeUrl = links.filter((l) => l?._attributes.rel === "alternate")[0]
						?._attributes?.href ?? "";
					const sourceURL = links.filter((l) => l?._attributes.rel === "self")[0]?._attributes
						?.href ?? "";
					const feedUrl = links.filter((l) => l?._attributes.rel === "self")[0]?._attributes
						?.href ?? "";
					return Promise.resolve({
						feedUrl,
						homeUrl,
