...
export const config: RouteConfig = {
	routeOverride: "/api/:tokType(u|t)-:token/:outputFmt(ast|json|html)/:composition",
};
...
const mergeNewItem = (
	...
	console.log("merging");
	const lookForIdx = (input.get(`${prefix}#${FORM_NEW_DATA_FUNCNAME}`) as string).split(
		".",
	)[1] ?? "0";
	const filteredFuncList = Object.entries(composition)
		.filter(([idx, elem]) => elem.fname === input.get(`${prefix}#${FORM_NEW_DATA_FUNCNAME}`));
	...
	if (filteredFuncList.length === 0) {
		...
		const foundInCompoIdx = filteredFuncList.findIndex(([key, elem]) => key === lookForIdx);
...
export const FormDataToParsedFunctionString = (
	...
	const funcBuildInputList = input.get(`${prefix}#${FORM_NEW_DATA_MERGESTRAT}`)?.toString()
			.toLowerCase() === "merge"
...
export const handler: Handlers = {
	GET: async (req, ctx) => {
		...
		const baseLink = `${protocol}//${host}/api/${tokType}-${token}/${outputFmt}/${composition}`;
		...
		if (parsedComp.left) {
			...
			if (parsedForm.left) {
				...
				return new Response(null, {
					...
					headers: {
						location: `/api/${tokType}-${token}/${outputFmt}/${parsedForm.right}`,
					},
				});
			}
		}
