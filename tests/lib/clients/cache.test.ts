...
import { cacheStack, renamerWithSha1, inMem as inMemCache } from "$lib/clients/cache.ts";
import { encodingWith, encoderMap } from '$lib/clients/cacheProviders/encoders/mod.ts'
...
Deno.test({
	...
	fn: async () => {
		...
		assert(cacheVal["content-encoding"] as string === 'base64url;id', `what we actually got is: ${cacheVal["content-encoding"]}`);
...
Deno.test({
	...
	// ignore: true,
	fn: async () => {
		const quickFoxTextLarger = Array(20).fill(`The quick brown fox jumps over the lazy dog! And that's a fact that you just have to deal with`).join(' ') // 494 chars
		const fscache = await fsCache.cache({ relativeDir: "./.fsCache", maxItems: 4 });
		const Hello = enc.encode("Hello World!")
