...
import { 
  awsV4Sig, toAmz, toDateStamp, hex
...
Deno.test("AWS V4 Sig", async () => {
  ...
  assert(signedReq.headers.has("x-amz-date"), 'Request should have the AMZ Date');
  ...
  assert(signedReq.headers.has("authorization"), 'Request should have the Authorization header');
  assert(signedReq.headers.get("authorization")?.startsWith('AWS4-HMAC-SHA256 Credential='), 'Request should start with the Authorization header');
  signedReq.headers.get("authorization")?.split(', ').forEach((tokenPair)=>{
    ...
    if(key==='Credential'){
      assert(value === `${initReqAwsSecretKey}/${toDateStamp(date)}/${initReqRegion}/${initReqService}/aws4_request`, 'SignedHeaders is incorrect')
    }
  })
});
...
Deno.test('hex encoding', ()=>{
  assert( 
    hex('deadbeef') === '6465616462656566', 'hex is incorrect'
  )
})