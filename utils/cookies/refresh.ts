...
import { decode, validate, verify, create, type Payload, type Header } from 'djwt'
...
export function stripHeaders(p?: Payload | null ): null | Record<string, unknown>{
    if(!p) return null
...
export const mintV1Cookie: CookieBakeryFn = (privCryptotKey: CryptoKey, kid: string, payload: Record<string, unknown> | null, iat: number = Math.round(Date.now()/1000) ) => {
...
// deno-lint-ignore no-unused-vars
const parseandValidateV1Token = (pair: CryptoKeyPair) => async (jwtStr:string ): Promise<Payload>=>{
    try{
...
export const confirmTokenGTEthreshold = (thresholdAgeSeconds: number) => (p:Payload): boolean | null => {
    const {iat, exp, v} = p
    if(v === 1 && iat && exp){
 ...
 */
 export const refreshCookieToken = (pair: CryptoKeyPair, kid: string, bakeryFn: CookieBakeryFn = mintV1Cookie) => async (headers: Headers | HeadersInit, findJwt: string ): Promise<{headers:Headers, jwt:string}> =>{
