...
import type {AbstractTokenFactory, VersionedPayload, LocalValidatorFns} from './tokenType.ts' 
import {
    create, 
    decode, 
...
export const isV1Token =  (async (payload: VersionedPayload) => payload?.v === 1) as LocalValidatorFns
export const isFromFeedCity = (async (payload: VersionedPayload) => payload?.iss === ISSUER) as LocalValidatorFns

export const v1: AbstractTokenFactory = (pair, kid:string, localValsFns = [isV1Token, isFromFeedCity] ) => {
    
    const v1Headers = {kid, typ: 'JWT', alg: 'ES384',  iss: ISSUER} as Header
    
    const _decode = async (jwtStr: string) => {
        try{
            const [headers, payload, signature] = decode(jwtStr) as [Header, Payload, Uint8Array]
    ...
    const validate =  async (jwtStr: string) => {
        ...
        try{
            ...
            const erroredUserVals = await Promise.all(
                localValsFns
                    .map(async (fn) => ({ fName: fn.name, val: await fn({headers, payload, signature}) }))
                    .filter(async nameVal => !(await nameVal).val)
            )

            if (erroredUserVals.length > 0)  {
                Promise.reject(new Error(erroredUserVals.map(({fName})=>fName).join(', ')))
            }
    ...
    return { mint, parse, validate, verify, v1Headers, v1Validators :[isV1Token, isFromFeedCity]}
}
