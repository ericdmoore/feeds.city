              ...
              <div class="mx-auto max-w-4xl">
                <dl class="rounded-lg bg-white shadow-lg sm:grid sm:grid-cols-3">
                  <div class="flex flex-col border-b border-gray-100 p-6 text-center sm:border-0 sm:border-r">
                    <dt class="order-2 mt-2 text-lg font-medium leading-6 text-gray-500">{first[0]}</dt>  
                    <dd class="order-1 text-5xl font-bold tracking-tight text-indigo-600">{first[1]}</dd>
                  </div>
                  {middle.map(([unit, numeral], i)=>{
                    return (
                      <div class="flex flex-col border-t border-b border-gray-100 p-6 text-center sm:border-0 sm:border-l sm:border-r">
                        <dt class="order-2 mt-2 text-lg font-medium leading-6 text-gray-500">{unit}</dt>
                        <dd class="order-1 text-5xl font-bold tracking-tight text-indigo-600">{numeral}</dd>
                      </div>)
                    })
                  }
                  <div class="flex flex-col border-t border-gray-100 p-6 text-center sm:border-0 sm:border-l">
                    <dt class="order-2 mt-2 text-lg font-medium leading-6 text-gray-500">{last[0]}</dt>
                    <dd class="order-1 text-5xl font-bold tracking-tight text-indigo-600">{last[1]}</dd>
                  </div>
